...
from datasets.dataset_ACDC import TwoStreamBatchSampler as TwoStreamBatchSampler_ACDC
from datasets.dataset_Kvasir import TwoStreamBatchSampler as TwoStreamBatchSampler_Kvasir
import ipdb
...
def trainer_acdc_dualmask_prompt_ssl_fixcoe_random_new_mean_up(args, model, snapshot_path, multimask_output, low_res):
    # Import based on dataset
    if 'Kvasir' in args.root_path or args.dataset == 'Kvasir':
        from datasets.dataset_Kvasir import Kvasir_dataset_aug as Dataset_train_cls, Kvasir_dataset as Dataset_val_cls, RandomGenerator
        TwoStreamBatchSampler = TwoStreamBatchSampler_Kvasir
        # For Kvasir, we use percentage-based labeled data (e.g., 5% = 0.05)
        use_percentage = True
    else:
        from datasets.dataset_ACDC import ACDC_dataset_aug as Dataset_train_cls, ACDC_dataset as Dataset_val_cls, RandomGenerator
        TwoStreamBatchSampler = TwoStreamBatchSampler_ACDC
    ...
    # max_iterations = args.max_iterations
    db_train = Dataset_train_cls(base_dir=args.root_path, split="train", num=None,
                           transform=transforms.Compose(
                               [RandomGenerator(output_size=[args.img_size, args.img_size], low_res=[low_res, low_res])]))
    
    db_val = Dataset_val_cls(base_dir=args.root_path, split="val")
    
    total_slices = len(db_train)
    
    # Calculate labeled slice based on dataset type
    if use_percentage:
        # For Kvasir: labeled_num is a percentage (e.g., 0.05 for 5%)
        labeled_slice = int(total_slices * args.labeled_num)
    else:
        # For ACDC: use patients_to_slices function
        labeled_slice = patients_to_slices(args.root_path, args.labeled_num)
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # [추가할 코드] 저해상도 라벨(128px)을 꺼내서 GPU로 보냅니다.
            low_res_label_batch = sampled_batch['low_res_label']
            low_res_label_batch = low_res_label_batch.cuda()
            ...
            supervised_loss1, loss_ce1, loss_dice1 = calc_loss_labeled(outputs1, low_res_label_batch, ce_loss, dice_loss, labeled_bs, args.dice_param)
            supervised_loss2, loss_ce2, loss_dice2 = calc_loss_labeled(outputs2, low_res_label_batch, ce_loss, dice_loss, labeled_bs, args.dice_param)
            ...
            if iter_num < args.warm_iter:
                # [Warm-up 구간] 손실값을 0으로 고정 (모델 실행 X)
                supervised_round2_loss1 = torch.tensor(0.0).cuda()
                loss_round2_ce1 = torch.tensor(0.0).cuda()
                loss_round2_dice1 = torch.tensor(0.0).cuda()
                supervised_round2_loss1_r = torch.tensor(0.0).cuda()
                loss_round2_ce1_r = torch.tensor(0.0).cuda()
                loss_round2_dice1_r = torch.tensor(0.0).cuda()
            else:
                # [본 학습 구간] 실제 모델 실행 및 손실 계산
                ...
                # 라벨은 low_res_label_batch 사용 (수정된 사항 유지)
                supervised_round2_loss1, loss_round2_ce1, loss_round2_dice1 = calc_loss_labeled(outputs_round2_1, low_res_label_batch, ce_loss, dice_loss, labeled_bs, args.dice_param)

                supervised_round2_loss1_r, loss_round2_ce1_r, loss_round2_dice1_r = calc_loss_labeled(outputs_round2_1_r, low_res_label_batch, ce_loss, dice_loss, labeled_bs, args.dice_param)
            ...
            # the third round
            if iter_num < args.warm_iter:
                supervised_round3_loss1 = torch.tensor(0.0).cuda()
                loss_round3_ce1 = torch.tensor(0.0).cuda()
                loss_round3_dice1 = torch.tensor(0.0).cuda()
                supervised_round3_loss1_r = torch.tensor(0.0).cuda()
                loss_round3_ce1_r = torch.tensor(0.0).cuda()
                loss_round3_dice1_r = torch.tensor(0.0).cuda()
                ...
                supervised_round3_loss1, loss_round3_ce1, loss_round3_dice1 = calc_loss_labeled(outputs_round3_2, low_res_label_batch, ce_loss, dice_loss, labeled_bs,args.dice_param)
                supervised_round3_loss1_r, loss_round3_ce1_r, loss_round3_dice1_r = calc_loss_labeled(outputs_round3_2_r, low_res_label_batch, ce_loss, dice_loss, labeled_bs,args.dice_param)
            ...
            if iter_num > 0 and iter_num % 200 == 0:  # evaluation
                ...
                for i_batch, sampled_batch in enumerate(valloader):
                    # valloader에서 이미지와 라벨을 가져옴
                    image = sampled_batch["image"] # (1, 3, 512, 512)
                    label = sampled_batch["label"] # (1, 512, 512)
                    
                    # 2D 검증 함수 호출
                    # Kvasir는 클래스가 2개(배경, 용종)이므로 classes=2로 전달
                    # (num_classes가 1로 설정되어 있다면 +1 해줘야 함)
                    metric_i = test_single_image_kvasir_mean(
                        image, 
                        label, 
                        model, 
                        classes=args.num_classes + 1,  # 배경 포함해서 2개
